...
class DataTrainingArguments:
    ...
    preprocessor_type: Optional[str] = field(
        default='utterance_generation_conditioned_on_strategy',
        metadata={
            "help": (
                "The type of preprocessor to use according to the downstream task "
            )
        },
    )
...
def main():
    ...
    print("using strategy list: ", strategy_list)
    ...
    preprocessor = InputPreprocessor(
        preprocessor_type=data_args.preprocessor_type,
        tokenizer=tokenizer,
    ...
    def get_belu_score(preds, labels):
        """
        calculates the bleu and rouge metrics
        """
        results = {}
        ...
        return {
            'B4': b4_sum / len(labels),
            'B3': b3_sum / len(labels),
            'B2': b2_sum / len(labels),
        }
    ...
    def compute_metrics(eval_preds):
        ...
        result_metrics = get_belu_score(preds=decoded_preds, labels=decoded_labels)
