...
class EmotionalIndex():
    def __init__(self, tokenizer:BartTokenizer, divide_num=8):
        self.vad_dict = self.get_vad_dict()
        self.special_token = list(tokenizer.special_tokens_map.values()) # 64
        self.symbol = ['?',',','.',':','!','\'','"'] # 65
    ...
    def get_value2index(self, one_item):
        for i in range(1, self.divide_num + 1):
            if i * 0.125 >= float(one_item):
                return i-1

    def get_vector2index(self, one_list):
        return self.get_value2index(one_list[0]) * self.divide_num + self.get_value2index(one_list[1])
    ...
    def get_one_sentence(self, sentence):
        ...
        for s in sentence:
            ...
            if s in self.special_token:
                emotion_list.append(self.divide_num * self.divide_num)
            elif s in self.symbol:
                emotion_list.append(self.divide_num * self.divide_num + 1)
            else:
                if s in self.vad_dict:
                    emotion_list.append(self.get_vector2index(self.vad_dict[s]))
                else:
                    emotion_list.append(self.divide_num * self.divide_num + 2)
...
class GenerateDataset(Dataset):
    ...
    def generate_truncate(self, history, max_len, flag='no_label'):  # label 不应该加cls标记， 因为decoder_inputs会自动加上 详见shift_tokens_right
        ...
        if flag == 'label':
            ...
            # todo: what does truncation of label do?
            input_ids = input_ids[:max_len]
            input_ids[-1] = self.tokenizer.eos_token_id
        else:
    ...
    def predict_truncate(self, history, max_len):
        input_ids = []
        for text in history:
            content = self.tokenizer.encode(text, add_special_tokens=False)
            input_ids.extend(content)
    ...
    def get_history_tensor(self, history):
        ...
        for sentence in history:
            if self.model_type < 2:
                tensor_history.append(torch.tensor(self.predict_truncate([sentence], max_len=self.each_length)).int())
            else:
                tensor_history.append(torch.tensor(self.generate_truncate([sentence], max_len=self.each_length)).int())
        tensor_history = tensor_history[-self.sentence_num:]
        for i in range(max(0, self.sentence_num - len(tensor_history))):
            tensor_history.append(torch.fill_(torch.zeros(self.each_length), self.tokenizer.pad_token_id).int())
        ans = torch.cat([tmp_t.unsqueeze(0) for tmp_t in tensor_history], dim=0)
    ...
    def get_norm_bert_input(self, tmp_dic):
        tmp_history = tmp_dic['history']
        input_ids = torch.tensor(self.predict_truncate(tmp_history, self.max_source_len))
    ...
    def get_norm_strategy_generate_input(self, tmp_dic):
        tmp_history = tmp_dic['history']
        # assert len(tmp_history) % 2 == 1, print(tmp_history)
        input_ids = torch.tensor(self.generate_truncate(tmp_dic['history'], self.max_source_len))
        labels = torch.tensor(self.generate_truncate([tmp_dic['future_strategy']], self.max_target_len, flag='label'), dtype=torch.long)
        return {
            "input_ids": input_ids,
            "attention_mask": input_ids != self.tokenizer.pad_token_id,
            "labels": labels
    ...
    def get_hierarchical_strategy_generate_input(self, tmp_dic):
        ...
        assert len(vads) == len(history_ids), print(len(vads), len(history_ids))
        input_ids = torch.tensor(self.generate_truncate(tmp_dic['history'], self.max_source_len))
        labels = torch.tensor(self.generate_truncate([tmp_dic['future_strategy']], self.max_target_len, flag='label'), dtype=torch.long)        # print('history_ids', history_ids.size())
        return {
            "input_ids": input_ids,
            "attention_mask": input_ids != self.tokenizer.pad_token_id,
            "history_ids": history_ids,
    ...
    def get_norm_sentence_generate_input(self, tmp_dic):
        input_ids = torch.tensor(self.generate_truncate(tmp_dic['history'], self.max_source_len))
        labels = torch.tensor(self.generate_truncate([tmp_dic['response']], self.max_target_len, flag='label'), dtype=torch.long)
        return {
            "input_ids": input_ids,
            "attention_mask": input_ids != self.tokenizer.pad_token_id,
            "labels": labels
    ...
    def get_norm_gpt_generate_input(self, tmp_dic):
        input_ids = torch.tensor(self.generate_truncate(tmp_dic['history'], self.max_source_len))
        labels = torch.tensor(self.generate_truncate([tmp_dic['strategy'] + ' ' + tmp_dic['response']], self.max_target_len, flag='label'), dtype=torch.long)
...
class GenerateDataset3(Dataset):
    ...
    def __init__(self, model_type, file_path, tokenizer, strategy2id=None, max_source_len=512, max_target_len=4, each_sentence_length=32, sentence_num=32, add_cause=False, with_strategy=False):
        ...
        for case_example in data:
            ...
            # initial history is emotion type + problem type + situation
            history = [_norm(emotion_type) + self.tokenizer.sep_token + _norm(
                problem_type) + self.tokenizer.sep_token + _norm(situation)]
            ...
            for index, tmp_dic in enumerate(dialog):
                ...
                if tmp_dic['speaker'] == 'sys' and tmp_dic['strategy'] != "Others":
                    ...
                    tmp_history = copy.deepcopy(history)
                    if with_strategy:
                        tmp_history[-1] = tmp_history[-1] + self.tokenizer.sep_token + tmp_strategy
                    self.total_data.append({
                        "history": tmp_history,
                ...
                if tmp_dic['speaker'] == 'sys':
                    ...
                    vad_list.append(np.zeros(3))
                    if with_strategy:
                        history.append(tmp_strategy + self.tokenizer.sep_token + text)
                    else:
                        history.append(text)
                else:
                    vad_list.append(np.array(tmp_dic['vad']))
    ...
    def generate_truncate(self, history, max_len, flag='train'):  # label 不应该加cls标记， 因为decoder_inputs会自动加上 详见shift_tokens_right
        input_ids = []
        for text in history:
            content = self.tokenizer.encode(text, add_special_tokens=False)
            input_ids.extend(content)
    ...
    def get_norm_strategy_generate_input(self, tmp_dic):
        ...
        assert len(tmp_history) % 2 == 1, print(tmp_history)
        input_ids = torch.tensor(self.generate_truncate(tmp_dic['history'], self.max_source_len))
        labels = torch.tensor(self.generate_truncate([tmp_dic['future_strategy']], self.max_target_len, flag='test'), dtype=torch.long)
        return {
            "input_ids": input_ids,
            "attention_mask": input_ids != self.tokenizer.pad_token_id,
            "labels": labels
    ...
    def get_norm_sentence_generate_input(self, tmp_dic):
        input_ids = torch.tensor(self.generate_truncate(tmp_dic['history'], self.max_source_len))
        labels = torch.tensor(self.generate_truncate([tmp_dic['response']], self.max_target_len, flag='test'), dtype=torch.long)
        return {
            "input_ids": input_ids,
            "attention_mask": input_ids != self.tokenizer.pad_token_id,
            "labels": labels
