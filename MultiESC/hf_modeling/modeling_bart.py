...
class BartForConditionalStrategyGeneration(BartPreTrainedModel):
    base_model_prefix = "model"
    _tied_weights_keys = ["encoder.embed_tokens.weight", "decoder.embed_tokens.weight", "lm_head.weight"]
    _keys_to_ignore_on_load_missing = ["final_logits_bias"]
    ...
    def __init__(self, config: BartConfig):
        super().__init__(config)
        self.model = BartModelForStrategy(config)
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))
        self.lm_head = nn.Linear(config.d_model, self.model.shared.num_embeddings, bias=False)
    ...
    def forward(
        ...
        dial_history_input_ids: Optional[torch.LongTensor] = None,
        dial_history_attention_mask: Optional[torch.Tensor] = None,
        #todo: define the type of this input
        user_states = None,
        ...
        outputs = self.model(
            ...
            dial_history_input_ids=dial_history_input_ids,
            dial_history_attention_mask=dial_history_attention_mask,
...
class BartModelForStrategy(BartPreTrainedModel):
    _tied_weights_keys = ["encoder.embed_tokens.weight", "decoder.embed_tokens.weight"]
    ...
    def forward(
        ...
        # todo: commented following lines in base paper impl
        # If the user passed a tuple for encoder_outputs, we wrap it in a BaseModelOutput when return_dict=True
        elif return_dict and not isinstance(encoder_outputs, BaseModelOutput):
            encoder_outputs = BaseModelOutput(
                last_hidden_state=encoder_outputs[0],
...
class MultiBartEncoder(BartPreTrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        # todo: configure it in the configuration file
